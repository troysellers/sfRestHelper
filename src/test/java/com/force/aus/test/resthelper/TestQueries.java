...
import com.force.aus.QueryResult;
...
public class TestQueries {
	...
	public void testQuery() throws JSONException {
		...
		QueryResult result = helper.runQuery(oAuth, "select id,name from account", oauthProps.getApiVersion());
		List<SObject> records = result.getRecords();
		
		logger.info("Is Done {}",result.isDone());
		logger.info("Returned [{}] records", result.getTotalSize());
		assertTrue(result.getResponseContent() != null && result.getResponseContent().length() > 0);
		assertTrue(records != null && records.size() > 0);
	...
	public void testGetSObject() throws JSONException, IOException {
		...
		for(SObject obj : response.getsObjects()) {
			...
			assertNotNull(objResponse.getRecordTypeInfos());
			break;
		}
	}
}
